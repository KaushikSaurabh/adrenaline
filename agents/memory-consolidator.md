...
- **RECURRENCE — failures only.** If the keeper is a failure (something broke, a command was denied, a fix had to be re-applied), ripgrep the store for that FAILURE's signature — the error text, the command, the symptom — BEFORE writing. If a fact already covers it, do NOT silently update it: append a dated line under a `## Recurrence` heading in that same file — `- recurred YYYY-MM-DD — <context> — <what fixed it this time>`. A repeat is the most valuable signal in the store; overwriting it destroys the only evidence that this trap is expensive.
- **PROMOTE ON THE 2ND RECURRENCE.** When a fact's `## Recurrence` list reaches 2 entries it has earned prevention: set `kind: lesson`, rewrite its index pointer TRIGGER-FIRST (the condition that PRECEDES the mistake, then the fix — e.g. "about to run `node -e` on a Windows path → write a .mjs file instead"), and add that pointer to the HOT `MEMORY.md` if absent. Flag it in your report as an always-fire candidate for the operator's `CLAUDE.md` — that promotion is the operator's call, not yours.
